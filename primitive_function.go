...
func (self *PrimitiveFunction) Apply(args *Data, env *SymbolTableFrame, extensionEnv ...*SymbolTableFrame) (result *Data, err error) {
	...
	var effectiveEnv *SymbolTableFrame
	if extensionEnv == nil {
		effectiveEnv = env
	} else {
		effectiveEnv = extensionEnv[0]
	}

	return (self.Body)(args, effectiveEnv)
}

func (self *PrimitiveFunction) ApplyWithoutEval(args *Data, env *SymbolTableFrame, extensionEnv ...*SymbolTableFrame) (result *Data, err error) {
	return self.Apply(QuoteAll(args), env, extensionEnv...)
}
